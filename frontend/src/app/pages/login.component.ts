    ...
    <div class="container" style="max-width:460px">
      ...
      @if (mode() === 'login' || mode() === 'register') {
        ...
        <div class="card">
          ...
          @if (googleEnabled()) {
            <a [href]="googleUrl()"><button style="width:100%;margin-bottom:10px">🔵 Sign in with Google</button></a>
          }
...
export class LoginComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (token) { this.auth.completeLogin(token); this.router.navigate(['/board']); return; }
  ...
  async usePasskey(): Promise<void> {
    ...
    try {
      ...
      this.auth.completeLogin(token); this.router.navigate(['/board']);
